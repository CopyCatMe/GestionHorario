<div class="mt-6 w-[95%] max-w-6xl mx-auto">
    ...
    @if ($usuario->horarios && $usuario->horarios->isNotEmpty())
        <div class="overflow-x-auto">
            <table
                class="min-w-full border border-gray-300 dark:border-gray-600 shadow-lg rounded-lg overflow-hidden text-center">
                <thead>
                    <tr class="bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-gray-300">
                        ...
                        @foreach (['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes'] as $dia)
                            <th class="px-4 py-3 border border-gray-300 dark:border-gray-600 ">
                                {{ $dia }}</th>
                        @endforeach
                    </tr>
                </thead>
                    ...
                    @foreach ($horas as $hora)
                        <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-gray-800 dark:even:bg-gray-900 ">
                            ...
                            @foreach (['Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes'] as $dia)
                                ...
                                @endphp
                                <td
                                    class="px-4 py-3 border border-gray-300 dark:border-gray-600 text-gray-800 dark:text-gray-300 ">
                                    @if ($horariosPorDia->isNotEmpty())
                                        @foreach ($horariosPorDia as $h)
                                            <span
                                                class="inline-block px-2 py-1 rounded-md text-sm 
    ...
    @else
        <p class="mt-3 text-gray-500 dark:text-gray-400 text0-center">No tiene horarios asignados.</p>
    @endif
</div>
